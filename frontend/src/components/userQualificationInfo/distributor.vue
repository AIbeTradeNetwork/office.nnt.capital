<script lang="ts">
  export default {
    name: 'QualificationsDistributor',
  }
</script>

<script setup lang="ts">
  import { EHideClasses } from 'types/enums'
  import { $isNotTMeMail } from 'utils/checks'
...
<template>
  <div class="p-6 flex flex-col gap-4">
    <div v-if="hideByRoute(EHideClasses.autotrade)" class="flex items-center">
      <div class="mobile:max-w-[55px] font-bold">
    ...
    <div v-if="hideByRoute(EHideClasses.autotrade)" class="-mt-4">
      ...
      <div v-if="$me.data.rank">
        <hr class="my-4" />

        <div class="text-base-content flex items-center gap-2">
          {{ $t('Rank') }}:
          <span class="font-bold">{{ $me.data.rank.code }}</span>
        </div>

        <div v-if="$me.data.plan" class="text-base-content">
          {{ $t('StartDate') }}:
          <span class="text-success">
            {{ $formatDate($me.data.rank.start_at, 'Pp') }}
          </span>
        </div>
        ...
        <div v-if="$me.data.plan" class="text-base-content">
          {{ $t('EndDate') }}:
          <span class="text-success">
            {{ $formatDate($me.data.rank.end_at, 'Pp') }}
          </span>
        </div>
      </div>

      <!-- <div v-if="$me.data.place">
      <div class="text-base-content flex items-center gap-2">{{ $t('Place') }}:</div>
      <div v-if="$me.data.plan" class="text-base-content">
        ref. UID:
        <span>
          {{ $me.data.place.match_uid }}
        </span>
      </div>
      <div class="text-base-content">
        {{ $t('Date') }}:
        <span class="text-success">
          {{ $formatDate($me.data.place.created_at, 'Pp') }}
        </span>
      </div>
      </div> -->

      <div v-if="$me.data.activity">
        <hr class="my-4" />
        <div class="text-base-content flex items-center gap-2">{{ $t('Activity') }}:</div>

        <div class="text-base-content">
          {{ $t('StartDate') }}:
          <span class="text-success">
            {{ $formatDate($me.data.activity.start_at, 'Pp') }}
          </span>
        </div>
        ...
        <div class="text-base-content">
          {{ $t('EndDate') }}:
          <span class="text-success">
            {{ $formatDate($me.data.activity.end_at, 'Pp') }}
          </span>
        </div>
      </div>
    </div>
